...
// MARK: - Stats View
// Shown in both Comic and Fanfic sidebars via the .stats section.
// Queries all local data for aggregate metrics — no network call needed.
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                statsCard(title: "Comics", systemImage: "book.closed.fill", index: 0) {
                    ...
                    statRow("Completed", value: "\(comics.filter { $0.progressPercent >= 1.0 }.count)")
                    statRow("Favourites", value: "\(comics.filter { $0.isFavorite }.count)")
                    statRow("Unread badges", value: "\(comics.filter { $0.newChapterCount > 0 }.count)")
                }

                statsCard(title: "Fan Fiction", systemImage: "scroll.fill", index: 1) {
                ...
                statsCard(title: "Bookmarks", systemImage: "bookmark.fill", index: 2) {
                    ...
                    statRow("Comic bookmarks", value: "\(bookmarks.filter { $0.contentType == "comic" }.count)")
                    statRow("Fanfic bookmarks", value: "\(bookmarks.filter { $0.contentType == "fanfic" }.count)")
                }
