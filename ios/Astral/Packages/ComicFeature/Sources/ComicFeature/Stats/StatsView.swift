...
struct StatsView: View {
    ...
    private var totalPagesViewed: Int {
        // Sum totalPages for chapters the user has actually read
    ...
    private var currentStreak: Int {
        ...
        // If no session today, check if yesterday had one (allow current day gap)
        ...
        VStack(spacing: 16) {
            ...
            HStack(spacing: 12) {
                ...
                odometerCard(
                    ...
                    label: "Best Streak",
                    icon: "trophy.fill",
                    sublabel: longestStreak > 0 ? "days" : nil
                )
            }
        }
        .padding(16)
    ...
    private var heatmapSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(alignment: .top, spacing: 3) {
                ...
                // Heatmap grid — 26 weeks x 7 days
                                ...
                                ForEach(0..<7, id: \.self) { day in
                                    ...
                                    if index < heatmapData.count {
                                        RoundedRectangle(cornerRadius: 2)
                                            .fill(heatmapColor(for: heatmapData[index]))
    ...
    private func buildHeatmapData() -> [Int] {
        ...
        // Sunday = 1, so offset = todayWeekday - 1 gives position in current week
        let totalDays = 26 * 7
    ...
    private var closestToFinishing: [AlmostDoneItem] {
        ...
        return (comicItems + fanficItems)
            .sorted { left, right in
                // Extract the number from "X ch left"
    ...
    private var breakdownSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // Fandom distribution (fanfics only)
            ...
            if !fandoms.isEmpty {
                ...
                Chart(fandoms, id: \.name) { fandom in
                    BarMark(
                        x: .value("Count", fandom.count),
                        y: .value("Fandom", fandom.name)
                    )
                    .foregroundStyle(AstralColors.gold.gradient)
                    .cornerRadius(4)
                    .annotation(position: .trailing, spacing: 4) {
    ...
    private func buildTimeline() -> [TimelineGroup] {
        ...
        // Group by month — show last 6 months max
